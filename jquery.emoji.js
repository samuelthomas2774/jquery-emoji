/* jQuery Emoji
 * https://github.com/samuelthomas2774/jquery-emoji
 * Modifications By HarryUK ;)
 * 
 * Like scrapture/jquery-emoji, but only replaces in text nodes.
 */
(function($) {
	// .emoji(), replaces any emoji in text nodes with images.
	$.fn.emoji = function(url) {
		// Set emoji url
		var old_url = $.emoji.url;
		if(typeof url == "string") $.emoji.url = url;
		
		var emojis = $.emoji(true);
		
		var r = $(this).each(function() {
			return $.emoji.textnodes(this).each(function() {
				var regex = new RegExp(':(' + emojis + '):', 'g');
				var textdata = $("<div></div>").text(this.data).html();
				$(this).replaceWith(textdata.replace(regex, $.emoji.replace));
			});
		});
	...
	$.emoji.replace = function() {
		...
		var src = $.emoji.url + emoji + ".png";
		return '<img class="emoji" src="' + src + '" alt="' + emoji + '" title="' + emoji + '" style="width: 20px; height: 20px;" />';
	};
})(jQuery);
